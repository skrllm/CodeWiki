...
- Если ваш ```enum``` содержит или может содержать не более 256 перечислений, то его следует наследовать от ```byte```.  
...
- При выборке коллекции путем ```LINQ``` запроса, следует проверять необходимость приведения полученного объекта типа ```IEnumerable<T>``` к типу ```List```.  
Приведение к типу ```List``` необходимо лишь для добавления или удаления элементов. Во всех остальных случаях можно использовать ```IEnumerable<T>```.
